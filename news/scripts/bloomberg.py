...
def run():
    ...
    # request中放入参数，请求头信息
    request = urllib.request.Request("https://www.bloomberg.com/markets", None, headers)
    # urlopen打开链接（发送请求获取响应）
    response = urllib.request.urlopen(request)
    if response.status == 200:
        body = response.read().decode("utf-8")
        body = body.split('<h3 data-component="title">Latest</h3>')[1].split(
            "zone_content__jaGly"
        )[0]
        ...
        for index in range(len(links)):
            ...
            if "/news/articles/" in link:
                elements = link.split('">')
                if len(elements) == 2:
                    posts.append({"title": elements[1], "link": base_url + elements[0]})
