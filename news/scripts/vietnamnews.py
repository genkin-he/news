...
# 栏目地址本身没变，一直是 https://vietnamnews.vn/economy-business-beat（连字符）；
# 曾按文章链接的斜杠形式推测过 /economy/business-beat，CI 实测该地址为 404，故不再尝试。
# 真正失效的是原选择器 .l-content article h2 a（站点改了 CSS 类名），改按文章 URL 模式
# 提取后同一地址可取到 21 条。
LIST_URLS = ("https://vietnamnews.vn/economy-business-beat",)
