...
"""
    ...
    def test_timeout_after_write_is_not_alerted(self):
        """已落盘后才超时的，不进告警通道；零产出的才报警"""
        util = SpiderUtil(notify=False)

        def slow_with_write():
            util.write_json_to_file([{"title": "t"}], "./tmp/test_timeout.json")
            time.sleep(1)

        def slow_without_write():
            time.sleep(1)

        with patch.object(util, "log_action_error") as mock_error:
            util.execute_with_timeout(slow_with_write, timeout=0.3)
            mock_error.assert_not_called()

        with patch.object(util, "log_action_error") as mock_error:
            util.execute_with_timeout(slow_without_write, timeout=0.3)
            mock_error.assert_called_once()
            self.assertIn("timeout", mock_error.call_args[0][0])

        if os.path.exists("./tmp/test_timeout.json"):
            os.remove("./tmp/test_timeout.json")

    def _health_setup(self):
        """把健康状态与告警文件都指到临时目录，返回一个可控的 SpiderUtil"""
        import shutil
        from news.scripts.util import spider_util

        shutil.rmtree("./tmp/health", ignore_errors=True)
        if os.path.exists("./tmp/action_errors.log"):
            os.remove("./tmp/action_errors.log")
        util = SpiderUtil()
        util.current_file = "health_test"
        return util, spider_util

    def _alerted(self):
        try:
            with open("./tmp/action_errors.log") as f:
                return f.read().strip()
        except FileNotFoundError:
            return ""

    def test_transient_failure_is_not_paged(self):
        """单轮失败不告警：CI 每几分钟一轮，境外站点偶发超时是常态"""
        util, _ = self._health_setup()
        util.log_action_error("request timeout")
        util.flush_action_errors()
        self.assertEqual(self._alerted(), "")

    def test_persistent_failure_is_paged_once_per_window(self):
        """连续失败超过宽限期才告警，且同一次故障不逐轮刷屏"""
        util, spider_util = self._health_setup()
        ...
        # 把失败起点前移到宽限期之外，这一轮应当告警
        health = util._read_health()
        health["first_failure"] = time.time() - spider_util.HEALTH_GRACE_SECONDS - 1
        util._write_health(health)
        util.log_action_error("request timeout")
        util.flush_action_errors()
        self.assertIn("request timeout", self._alerted())
    ...
    def test_success_clears_failure_streak(self):
        ...
        util.log_action_error("request timeout")
        util.flush_action_errors()
        self.assertIsNotNone(util._read_health().get("first_failure"))

        util.flush_action_errors()  # 无错误 = 成功
        health = util._read_health()
        self.assertIsNone(health.get("first_failure"))
        self.assertIsNotNone(health.get("last_success"))

    def test_read_with_deadline(self):
        """分片很慢时，读取要在总预算内主动放弃，而不是无限拉长"""
        ...
        util = SpiderUtil(notify=False)
        body = util.read_with_deadline(SlowResponse([b"ab", b"cd"], 0), 1)
        self.assertEqual(body, b"abcd")

        with self.assertRaises(TimeoutError):
            util.read_with_deadline(SlowResponse([b"ab"] * 20, 0.05), 0.1)

    def test_request_timeout(self):
        """测试 request_timeout 会把请求超时收敛进本轮剩余预算"""
        # 不在 execute_with_timeout 内时没有预算约束，原样返回
        util = SpiderUtil(notify=False)
        self.assertEqual(util.request_timeout(30), 30)

        captured = []

        def collect():
            # 预算 5 秒：preferred 大于剩余预算时被压到 (剩余 - reserve)
            captured.append(util.request_timeout(30, reserve=1))
            # preferred 小于剩余预算时保持不变
            captured.append(util.request_timeout(2, reserve=1))

        util.execute_with_timeout(collect, timeout=5)
        self.assertAlmostEqual(captured[0], 4, delta=0.5)
        self.assertEqual(captured[1], 2)

        # 预算耗尽时不返回 0 或负数，保证请求仍有最小可用超时
        util._deadline = time.time() - 10
        self.assertEqual(util.request_timeout(30, minimum=1.5), 1.5)

        # requests 的 (connect, read) 二元组逐项收敛后原样返回
        self.assertEqual(util.request_timeout((2, 5), minimum=1.5), (1.5, 1.5))
