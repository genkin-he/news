...
# execute_with_timeout 的预算为 10 秒、CI 另有 gtimeout 15 秒兜底，原来两处固定
# timeout=10 单独一次就能吃掉整轮预算（CI 日志里的 "Read timed out. (read timeout=10)"
# 与随后的 "timeout: still running after 10s, aborted"）。
# 同时去掉 proxies=util.get_random_proxy()：该池只提供 http 键，requests 对 https
# 请求不会使用它（本脚本全部是 https），实测池内 36 个代理已全部不可达，留着只会误导。
REQUEST_TIMEOUT = (3, 6)  # (connect, read)；再由 request_timeout 按剩余预算收敛
...
def get_detail(link):
    ...
    if response.status_code == 200:
        ...
        soup = body.select_one('div.techi-post-content')
        if not soup:
            util.error("article content not found: {}".format(link))
            return ""
        ad_elements = soup.select("script, style, iframe, noscript, section, aside, div")
...
def run():
    ...
    try:
        ...
        url = "https://www.techi.com/category/markets-equities/feed.xml"
        response = session.get(
            url,
            headers=headers,
            timeout=util.request_timeout(REQUEST_TIMEOUT),
        ...
        if response.status_code == 200:
            ...
            for item in rss_items:
                if data_index >= 3 or util.out_of_time():
                    break
                link = item['link']
                title = item['title']
