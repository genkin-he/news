...
# execute_with_timeout 的预算为 10 秒、CI 另有 gtimeout 15 秒兜底。原实现三处 urlopen
# 都没设 timeout（默认无限等待），而一轮最多要发 1 + 3×2 = 7 次请求，站点一慢就被
# 外层强杀且零产出（日志里的 "timeout: still running after 10s, aborted"）。
REQUEST_TIMEOUT = 2
...
def get_detail(link, source):
    ...
    if response.status == 200:
        ...
        # 摘要容器缺失时跳过该条，原实现直接取 [0] 会抛 IndexError 并中断整轮
        abstruct = lxml.select(".abstruct-info")
        if not abstruct:
...
def run():
    ...
    if response.status == 200:
        ...
        for node in nodes:
            if post_count >= 3 or util.out_of_time():
                break
