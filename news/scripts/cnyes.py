...
MAX_NEW_POSTS = 3  # 1 次列表 + 最多 3 次详情，最坏 8 秒

def get_detail(link):
    util.info("link: {}".format(link))
    try:
        response = requests.get(link, headers=headers, timeout=util.request_timeout(REQUEST_TIMEOUT))
...
def run():
    ...
    for index in range(len(items)):
        if index > 5 or collected >= MAX_NEW_POSTS or util.out_of_time():
            break
