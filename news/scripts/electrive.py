...
def run():
    ...
    # CI 实测：前 5 个 feed 正常、最后一个 short-circuit 返回 403（响应体为 Cloudflare
    # 1xxx 错误页），符合限流特征——此前一轮内已连打 5 个 feed 与多个详情页。
    # 各 feed 之间加一个小间隔即可，总开销远低于 execute_with_timeout 的预算。
