        ...
        if resp.status == 200:
            ...
            for paragraph in paragraphs:
                if paragraph["type"] == "paragraph":
                    ...
                    if len(paragraph["content"]) > 0 and len(paragraph["content"][0]["content"]) > 0 and len(paragraph["content"][0]["content"][0]["content"]) > 0:
                        article_body = article_body + "<ul><li><p>{}</p></li></ul>".format(paragraph["content"][0]["content"][0]["content"][0]["text"])
                else:
...
def run():
    ...
    if response.status == 200:
        ...
        print("infoq request error: ", response)
