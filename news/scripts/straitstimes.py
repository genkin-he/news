...
def get_detail(link):
    ...
    try:
        ...
        if response.status_code == 200:
            ...
            # 移除相关文章容器的所有同级元素
            related_container = soup.select_one(".related-article-container")
            if related_container:
                for sibling in list(related_container.next_siblings):
                    if sibling:
                        sibling.decompose()

            ad_elements = soup.select("style, script,.ads-mid-article,.related-article-container")
    ...
    if response.status_code == 200:
        ...
        for index in range(len(items)):
            ...
            if link in ",".join(_links):
                util.info("exists link: {}".format(link))
                continue
            title = items[index].select_one(".card-title").text.strip()
