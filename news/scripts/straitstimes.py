...
def get_detail(link):
    ...
    try:
        ...
        if response.status_code == 200:
            ...
            soup = body.select_one(".storyline-wrapper")
...
def run(url):
    ...
    if response.status_code == 200:
        ...
        items = soup.select(".container a.select-none.items-start")
        for index in range(len(items)):
            if index > 2:
                break
            stretched_link = items[index]
            ...
            if link in ",".join(_links):
                util.info("exists link: {}".format(link))
                continue
            title = items[index].select_one("h4").text.strip()
