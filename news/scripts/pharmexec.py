...
def get_detail(link):
    ...
    if response.status == 200:
        ...
        print("pharmexec request: {} error: ".format(link), response)
        return ""
    ...
    if response.status == 200:
        ...
        for index in range(len(items)):
            ...
            if link in ",".join(_links):
                print("pharmexec exists link: ", link)
                break
        ...
        util.log_action_error("pharmexec request error: {}".format(response))
