...
# execute_with_timeout 的预算为 10 秒、CI 另有 gtimeout 15 秒兜底，故单次请求的
# timeout 与本轮请求数的乘积必须落在预算内。
REQUEST_TIMEOUT = (2, 3)  # (connect, read)；1 次列表 + 最多 3 次详情
...
class QQNewsSpider:
    def __init__(self):
        ...
        # 1 次列表 + 最多 batch_size 次详情；配合 REQUEST_TIMEOUT 须落在
        # execute_with_timeout 的 10 秒预算内（原为 5 且 timeout=REQUEST_TIMEOUT，最坏 60 秒）
        self.batch_size = 3
    ...
    def get_detail(self, link: str) -> str:
        ...
        try:
            ...
            response = requests.get(link, headers=self.headers, timeout=REQUEST_TIMEOUT)
