...
# execute_with_timeout 的预算为 10 秒、CI 另有 gtimeout 15 秒兜底。原实现的两处
# urlopen 均未设 timeout（默认无限等待），且下方循环遍历接口返回的全部条目、
# 每条新文章都发一次详情请求、没有任何上限——平时多为 exists 故无碍，新文章一多
# 就爆预算，日志表现为 "timeout: still running after 10s, aborted" 且零产出。
REQUEST_TIMEOUT = 2  # 1 次列表 + 最多 MAX_NEW_POSTS 次详情，最坏 8 秒
MAX_NEW_POSTS = 3
