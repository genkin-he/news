...
def _safe_get(url, **kwargs):
    ...
    #
    # news.yml 注入的 seekingalpha secret 作为 cookie 会话过期后，端点会直接 403
    # （CI 日志里 seekalpha_articles 同一 cookie 固定 403、去掉 cookie 重试即 200）。
    # 本脚本的 403 也是零星出现而非持续，故与 seekalpha_articles 采用同一策略：
    # 先按原样请求，非 200 再去掉 cookie 重试一次。
...
def run():
    ...
    try:
        # 原值 30 远超 execute_with_timeout 的 10 秒预算，慢一次即被外层强杀
        response = _safe_get(url, headers=headers, timeout=util.request_timeout(6))
