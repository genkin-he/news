...
# (connect, read)：requests 对两者分别计时，给单一数值会让最坏耗时翻倍
REQUEST_TIMEOUT = (3, 5)  # 单次请求
...
def run(url):
    try:
        ...
        if not response:
            # 运行级失败：整轮取不到任何文章，需进入告警通道
            util.log_action_error("failed to get response from main page")
            return
        ...
        util.log_action_error("error in run function: {}".format(str(e)))
        traceback.print_exc()

link1 = "https://www.businesswire.com/newsroom?language=en&industry=1000178%7C1778661"
if __name__ == "__main__":
    # 停止采集：站点由 Akamai Bot Manager 拦截（server: AkamaiGHost，响应体为
    # "Page Unavailable"），curl_cffi 的 chrome131 / safari18_0 / firefox133 全档位均返回
    # 403，换指纹无效；CI 上则表现为连接挂起、每轮固定产生一条告警。代码层面无解。
    # 保留一条 pass 使 if 块非空——news.yml 执行前会 chmod +x 所有脚本，
    # 移除可执行位不起作用，pass 才是有效的停用方式。
    pass
