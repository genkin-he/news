...
def run(url, filename):
    ...
    util.info("filepath: {}".format(filepath))
    ...
    if response.status == 200:
        ...
        if len(articles) > 0 and insert:
            if filename == "author_tipranks_auto_generated_intelligence_newsdesk" or filename == "author_tipranks_newsdesk" or filename == "author_tipranks_hongkong_newsdesk":
                if len(articles) > 40:
...
if __name__ == "__main__":
    util.execute_with_timeout(run, "https://www.tipranks.com/api/news/posts?per_page=10&category=article", "others")

    util.execute_with_timeout(run, "https://www.tipranks.com/api/news/posts?per_page=10&category=global-markets", "category_global_markets")

    util.execute_with_timeout(run, "https://www.tipranks.com/api/news/posts?per_page=10&topic=dividend-stocks", "topic_dividend_stocks")

    util.execute_with_timeout(run, "https://www.tipranks.com/api/news/posts?per_page=20&author=tipranks-auto-generated-intelligence-newsdesk", "author_tipranks_auto_generated_intelligence_newsdesk")

    util.execute_with_timeout(run, "https://www.tipranks.com/api/news/posts?per_page=20&author=tipranksnewsdesk", "author_tipranks_newsdesk")

    util.execute_with_timeout(run, "https://www.tipranks.com/api/news/posts?per_page=20&author=tiprankshongkongnewsdesk", "author_tipranks_hongkong_newsdesk")
