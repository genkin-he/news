...
def get_detail(link):
    if link in current_links:
        return ""
    print("pingwest_status link: ", link)
    ...
    if response.status == 200:
        ...
        print("pingwest_status request: {} error: ".format(link), response)
        return ""
    ...
    if response.status == 200:
        ...
        for index in range(len(items)):
            ...
            if link in ",".join(_links):
                print("pingwest_status exists link: ", link)
                break
        ...
        print("pingwest_status request error: ", response)

try:
    run("https://www.pingwest.com/api/state/list")
except Exception as e:
    print("pingwest_status exec error: ", repr(e))
    log_action_error(f"pingwest_status exec error: {repr(e)}\n")
