    ...
    if response.status == 200:
        ...
        for index in range(len(posts)):
            if index < 1:
                ...
                pub_date = parse_time(post["date"], "%Y-%m-%dT%H:%M:%S.%fZ")
        ...
        print("tipranks announcements request error: ", response)


try:
    run()
except Exception as e:
    print("tipranks announcements exec error: ", repr(e))
    traceback.print_exc()
    log_action_error(f"tipranks announcements exec error: {repr(e)}\n")
