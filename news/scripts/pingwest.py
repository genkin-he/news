...
def run(link):
    ...
    if response.status == 200:
        ...
        list_html = data.get("list") if data else None
        if not list_html:
            return

        soup = BeautifulSoup(list_html, "lxml")
        # 只处理正文条目，跳过广告 article（class 含 bd）
        all_articles = soup.select("article")
        items = [el for el in all_articles if "bd" not in (el.get("class") or [])]
...
if __name__ == "__main__":
    util.execute_with_timeout(run, "https://www.pingwest.com/api/index_news_list?last_id=")
