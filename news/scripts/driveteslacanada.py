...
def get_detail(link):
    if link in current_links:
        return ""
    print("driveteslacanada link: ", link)
    ...
    try:
        ...
        if response.status_code == 200:
            ...
            print("driveteslacanada request: {} error: ".format(link), response.status_code)
            return ""
    except Exception as e:
        print("driveteslacanada request failed:", str(e))
        return ""
...
def run(link):
    ...
    if response.status_code == 200:
        ...
        items = soup.select(".news > article")
        for index in range(len(items)):
            ...
            if link in ",".join(_links):
                print("driveteslacanada exists link: ", link)
                break
        ...
        util.log_action_error("driveteslacanada request error: {}".format(response.status_code))
