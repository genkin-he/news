...
# 裸 requests 对该站固定 403（本地实测），而 curl_cffi 的 chrome131 / safari18_0 /
# firefox133 均返回 200——CI 上表现为请求挂起而非 403，属同一拦截的另一种表现。
IMPERSONATE = "safari18_0"
REQUEST_TIMEOUT = 6  # 列表 1 次；详情另计
MAX_NEW_POSTS = 4  # 每轮最多新收 4 篇（原为最多 14 篇）
...
def get_detail(link):
    ...
    response = curl_requests.get(link, headers=headers, impersonate=IMPERSONATE, timeout=REQUEST_TIMEOUT)
...
def run(url):
    ...
    response = curl_requests.get(url, headers=headers, impersonate=IMPERSONATE, timeout=REQUEST_TIMEOUT)
    if response.status_code == 200:
        response.encoding = 'utf-8'
        body = response.text
        ...
        for var_name in variable_names:
            # parse_article 内部会调 get_detail，7 个变量各取 2 篇即最多 14 次详情请求，
            # 实测整轮约 12 秒、超出 execute_with_timeout 的 10 秒预算。此处按已收录数与
            # 剩余预算双重收口：宁可少抓几篇，也要保住已抓到的内容。
            if collected >= MAX_NEW_POSTS or util.out_of_time():
                break
