...
def run(url):
    ...
    if response.status == 200:
        ...
        news_data = data["props"]["pageProps"]["newsData"]
        for post in news_data:
            if post_count >= 2:
                break
            # isLiveBlog
            if post["isLiveBlog"] == True:
                continue
            link = base_url + str(post["article_url"].strip())
            title = str(post["heading1"].strip())
            if title == "":
                continue
            id = str(post["article_id"])
